...
async function checkExpiryWarnings(client) {
  ...
  stmt.free();
  const expiringSoon = rows.filter(t => { const unix = extractUnix(t.deadline); return unix && unix > now && unix <= cutoff; });
  for (const task of expiringSoon) {
    ...
    for (const claimant of claimants) {
      try {
        ...
        await user.send([`${urgencyEmoji(task.deadline)} **Deadline Warning!**`, ``, `Your task **${task.title}** (${cat.emoji} ${task.category}) expires ${countdown}.`, ``, `Submit now with \`/submit\`!`].join('\n'));
...
async function checkExpiredTasks(client) {
  ...
  while (stmt.step()) rows.push(stmt.getAsObject());
  stmt.free();
  const expired = rows.filter(t => { const unix = extractUnix(t.deadline); return unix && unix <= now; });
  for (const task of expired) {
    ...
    for (const claimant of claimants) {
      try {
        const user = await client.users.fetch(claimant.user_id);
        await user.send([`⛔ **Task Expired — ${task.title}**`, ``, `Your task has expired and been removed. Keep an eye out for new tasks!`].join('\n'));
      } catch (err) { console.error(`[Deadline] Could not DM ${claimant.username}:`, err.message); }
    }
    ...
    if (logChannelId) {
      try {
        ...
        await logChannel.send({ embeds: [new EmbedBuilder().setTitle(`⛔  Task Expired`).setColor(0xED4245).addFields({ name: 'Task', value: `#${task.id} — ${task.title}`, inline: true }, { name: 'Category', value: `${cat.emoji} ${task.category}`, inline: true }, { name: 'Claimants notified', value: `${claimants.length}`, inline: true }).setTimestamp()] });
      } catch (err) { console.error('[Deadline] Could not post expiry log:', err.message); }
    }
    await removeTask(task.id);
    console.log(`[Deadline] Task #${task.id} expired and removed`);
  }
}
