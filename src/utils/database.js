...
// ── Cubs graduation tracking ──────────────────────────────────────────────────

export async function getApprovedSubmissionCount(userId) {
  await getDb();
  // Only count approved submissions for CUBS category tasks
  const row = queryOne(`
    SELECT COUNT(*) as count FROM submissions s
    JOIN tasks t ON t.id = s.task_id
    WHERE s.user_id = ? AND s.status = 'approved' AND t.category = 'cubs'
  `, [userId]);
  return row?.count ?? 0;
}
