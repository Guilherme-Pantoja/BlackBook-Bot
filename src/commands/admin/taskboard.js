...
export async function execute(interaction) {
  ...
  for (const [catKey, cat] of Object.entries(CATEGORIES)) {
    ...
    for (const t of catTasks) {
      ...
      const claimNames = claims.map(c => c.username);
      const displayNames = claimNames.length > 10
        ? claimNames.slice(0, 10).join(', ') + ` +${claimNames.length - 10} more`
        : claimNames.join(', ');
