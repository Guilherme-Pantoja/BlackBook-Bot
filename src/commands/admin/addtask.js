  ...
  .addSubcommand(sub =>
    sub.setName('template')
      ...
      .addStringOption(opt => opt.setName('type').setDescription('Which template?').setRequired(true).addChoices(
        ...
        { name: '🐾 Cubs Task', value: 'cubs_task' },
      ))
...
export async function execute(interaction) {
  ...
  if (sub === 'custom') {
    ...
    modal.addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('category').setLabel('Category (community/outreach/content/cubs)').setStyle(TextInputStyle.Short).setPlaceholder('community').setRequired(true)),
