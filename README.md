...
## Deadline Formats
Deadlines are converted to Discord's native live countdown format automatically:
- `48h` or `48 hours`
- `2d` or `2 days`
- `1w` or `1 week`
- `April 27`

Discord renders these as **"in 2 days"** or **"3 hours ago"** in each user's own timezone.
...
## Versions
- **v1.0.0** — Core bot, task management, claim and submit flow, review system
- **v1.1.0** — Live deadline countdowns, auto-expiry, 2-hour warning DMs
- **v1.2.0** — `/unclaim` command, `/cleartasks` confirmation prompt, database migration fix
- **v1.3.0** — Discord native timestamps, `/mytasks` command, duplicate claim prevention, deleted task graceful handling, button numbers match list
